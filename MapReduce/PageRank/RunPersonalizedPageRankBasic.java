...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapClass extends
    ...
    // Source Nodes
    private static final ArrayList<Integer> src = new ArrayList<>();
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacencyList().size() > 0) {
        ...
        ArrayListOfFloatsWritable rank = node.getPageRank();

        ArrayListOfFloatsWritable mass = new ArrayListOfFloatsWritable();
        for (int i = 0; i < src.size(); i++){
          mass.add(rank.get(i) - (float) StrictMath.log(list.size()));
        }
  ...
  private static class CombineClass extends
    ...
    private static final PageRankNode intermediateMass = new PageRankNode();
    private static final ArrayList<Integer> src = new ArrayList<Integer>();
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> values, Context context)
      ...
      for (PageRankNode n : values) {
        if (n.getType() == PageRankNode.Type.Structure) {
          ...
          for (int i = 0; i < numSources; i++) {
            mass.set(i, sumProbs(mass.get(i), n.getPageRank().get(i)));
          }
          massMessages++;
        }
      }
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      while (values.hasNext()) {
        ...
        if (n.getType().equals(PageRankNode.Type.Structure)) {
          ...
          for (int i = 0; i < numSources; i++) {
            massList.set(i, sumProbs(massList.get(i), n.getPageRank().get(i)));
          }
      ...
      if (structureReceived == 1) {
        ...
        for(int i = 0; i < numSources; i++) {
          totalMass.set(i, sumProbs(totalMass.get(i), massList.get(i)));
        }
  ...
  private static class MapPageRankMassDistributionClass extends
      Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode> {
        private static ArrayList<Float> missingMass = new ArrayList<>();
    ...
    public void setup(Context context) throws IOException {
      ...
      for (String node : conf.getStrings(NODE_SRC)) {
        src.add(Integer.valueOf(node));
      }
      for (String missing : conf.getStrings("MissingMass")) {
        missingMass.add(Float.valueOf(missing));
      }
    }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      for (int i = 0; i < src.size(); i++) {
        if (Integer.valueOf(src.get(i)) == nid.get()){
          float jump = (float) Math.log(ALPHA);
          float link = (float) Math.log(1.0f - ALPHA)
                      + sumProbs(p.get(i), (float) Math.log(missingMass.get(i)));
          p.set(i, sumProbs(jump, link));
        } else {
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes, String sources) throws Exception {
    ...
    for (int n = 0; n < mass.size(); n++) {
      float loss = 1.0f - (float) StrictMath.exp(mass.get(n));
        missing.add(loss < 0.0f ? 0.0f : loss);
    }
  ...
  private ArrayList<Float> phase1(int i, int j, String basePath, int numNodes, String sources) throws Exception {
    ...
    job.getConfiguration().set(NODE_SRC, sources);
    ...
    ArrayList<Float> mass = new ArrayList<>(Collections.nCopies(numSources, Float.NEGATIVE_INFINITY));

    FileSystem fs = FileSystem.get(getConf());
    for (FileStatus f : fs.listStatus(new Path(outm))) {
      FSDataInputStream fin = fs.open(f.getPath());
      for (int g = 0; g < numSources; g++) {
        mass.set(g, sumProbs(mass.get(g), fin.readFloat()));
      }
      fin.close();
    }
  ...
  private void phase2(int i, int j, ArrayList<Float> missing, String basePath, int numNodes, String sources) throws Exception {
    ...
    for (int k = 0; k < missing.size(); k++){
      missingMass += String.valueOf(missing.get(k));
      if (k < missing.size() - 1 ) {
        missingMass += ",";
      }
    }
